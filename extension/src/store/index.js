...
export default createStore({
  ...
  getters: {
    ...
    activeTransactionCount: (state) =>
      state.transactions.filter(
        (tx) => tx.status === "processing" || tx.status === "validating",
      ).length,

    completedTransactionCount: (state) =>
      state.transactions.filter((tx) => tx.status === "completed").length,

    failedTransactionCount: (state) =>
      state.transactions.filter((tx) => tx.status === "failed").length,

    leaderNodes: (state) =>
      state.nodes.filter((node) => node.role === "leader"),

    validatorNodes: (state) =>
      state.nodes.filter((node) => node.role === "validator"),

    extensionNodes: (state) =>
      state.nodes.filter((node) => node.role === "extension"),

    totalMempoolSize: (state) => {
      const mempool = state.mempoolData;
      return (
        (mempool.rawTx?.length || 0) +
